...
export const AnimationCard: React.FC<AnimationCardProps> = ({ 
  ...
  const { filteredFound, filteredMissing } = useMemo(() => {
    ...
    const match = (item: { path: string, bonePath: string, isLocalScaleOverridden?: boolean, showSkinLabel?: boolean, isOverridden?: boolean }) => {
      // STRICT SEARCH: Match against complete attachment path string, ignore bone path
      const attachmentPath = (item.path || "").toLowerCase();
  ...
  const cardId = `search-anim-${result.skeletonName || "Unknown"}-${result.animationName}`;
                ...
                <div className="space-y-2">
                  ...
                  <ul className="space-y-2">
                    {filteredMissing.map((img, idx) => (
                      <li 
                        key={`${img.path}-${idx}`} 
                        id={`search-asset-${result.skeletonName || "Unknown"}-${result.animationName}-${img.path}`}
                        className="flex items-start gap-3 px-3 py-3 text-sm text-red-300 rounded bg-red-900/20 border border-red-900/30"
                      >
                  ...
                  <ul className="space-y-2">
                    {filteredFound.map((img, idx) => {
                      ...
                      return (
                        <li 
                          key={`${img.path}-${idx}`} 
                          id={`search-asset-${result.skeletonName || "Unknown"}-${result.animationName}-${img.lookupKey}`}
                          ref={(el) => {
