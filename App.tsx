...
import { Activity, Layers, Search, X, Zap, CheckSquare, RotateCcw, Download, Upload, Film, AlertTriangle, ChevronUp, ChevronDown } from 'lucide-react';

type SortKey = 'path' | 'originalSize' | 'maxRenderSize' | 'sourceAnimation' | 'sourceSkeleton';

interface SearchMatch {
  type: 'global' | 'animation' | 'asset';
  elementId: string;
  skeletonName?: string;
...
export default function App() {
  ...
  const lastScrolledTerm = useRef(""); // Track the term used for the last auto-scroll
  ...
  const filteredResults = useMemo(() => {
    ...
    return results.filter(result => {
      ...
      const assetMatch = (img: any) => {
        // Match against full attachment path but exclude bone path
        const attachmentPath = (img.path || "").toLowerCase();
        const textMatch = attachmentPath.includes(term);
  ...
  const filteredGlobalStats = useMemo(() => {
    ...
    if (debouncedSearchTerm.trim()) {
      ...
      stats = stats.filter(stat => {
        // Match against full attachment path but exclude bone path
        const attachmentPath = (stat.path || "").toLowerCase();
        const textMatch = attachmentPath.includes(term) || stat.sourceAnimation.toLowerCase().includes(term);
  ...
  const scrollToMatch = (match: SearchMatch) => {
    // GUARD CLAUSE: Strictly block scroll when query is empty or no match provided
    if (!searchTerm.trim() || !match) return;

    // 1. Trigger expansion if needed
    if (match.type === 'animation' || match.type === 'asset') {
      setExpandTrigger({
        name: match.animationName || "",
        skeletonName: match.skeletonName || "Unknown Skeleton",
        assetKey: match.assetKey,
        ts: Date.now()
      });
    }

    // 2. Wait for layout to settle (especially if expanding)
    const delay = match.type === 'global' ? 0 : 150;
    
    setTimeout(() => {
      const element = document.getElementById(match.elementId);
      if (element) {
        // Custom Highlight Cleanup: remove from any other active highlights
        document.querySelectorAll('.ring-spine-accent').forEach(el => {
            el.classList.remove('ring-2', 'ring-spine-accent', 'ring-offset-4', 'ring-offset-gray-900', 'z-10');
        });

        // 3. Custom Scroll Logic (Sticky Header Fix)
        const headerOffset = 180;
        ...
        // 4. Brief Visual Highlight (Skip for 'global' matches)
        if (match.type !== 'global') {
          element.classList.add('ring-2', 'ring-spine-accent', 'ring-offset-4', 'ring-offset-gray-900', 'z-10');
          setTimeout(() => {
            element.classList.remove('ring-2', 'ring-spine-accent', 'ring-offset-4', 'ring-offset-gray-900', 'z-10');
          }, 1500);
        }
      }
    }, delay);
  ...
  // Instantly clear result counts when searchTerm is emptied to improve UX
  useEffect(() => {
    if (searchTerm === "") {
        setSearchMatches([]);
        setCurrentMatchIndex(0);
  ...
  // LINEARIZE SEARCH MATCHES (Visual Order)
  useEffect(() => {
    const term = debouncedSearchTerm.trim().toLowerCase();

    // STICKY VIEWPORT ON CLEAR: If query is empty, clear matches and return early to avoid scrolling
    if (!term || filteredResults.length === 0) {
      setSearchMatches([]);
      setCurrentMatchIndex(0);
      lastScrolledTerm.current = ""; // Reset anchoring
      return; 
    }

    const newMatches: SearchMatch[] = [];

    // Phase 1: Global Stats (Top)
    filteredGlobalStats.forEach(stat => {
      newMatches.push({
        type: 'global',
        elementId: `search-global-${stat.lookupKey}`,
        assetKey: stat.lookupKey
      });
    });

    // Phase 2: Skeleton Groups (Bottom)
    groupedResults.forEach(([skeletonName, animations]) => {
      animations.forEach(res => {
        // 1. Check animation name itself
        if (res.animationName.toLowerCase().includes(term)) {
          newMatches.push({ 
            type: 'animation',
            elementId: `search-anim-${skeletonName}-${res.animationName}`,
            skeletonName, 
            animationName: res.animationName 
          });
        }

        // 2. Check individual matching assets
        const checkAsset = (img: any) => {
          const attachmentPath = (img.path || "").toLowerCase();
          return attachmentPath.includes(term);
        };

        res.foundImages.forEach(img => {
          if (checkAsset(img)) {
            newMatches.push({
              type: 'asset',
              elementId: `search-asset-${skeletonName}-${res.animationName}-${img.lookupKey}`,
              skeletonName,
              animationName: res.animationName,
              assetKey: img.lookupKey
            });
          }
        });

        res.missingImages.forEach(img => {
          if (checkAsset(img)) {
            newMatches.push({
              type: 'asset',
              elementId: `search-asset-${skeletonName}-${res.animationName}-${img.path}`,
              skeletonName,
              animationName: res.animationName,
              assetKey: img.path
            });
    ...
    // Only auto-scroll to Match #0 IF it's a "Fresh Search" (user just typed something new)
    // This prevents the viewport from jumping back to match #0 when background data updates
    // while the user is actively navigating through the match list.
    const isFreshSearch = debouncedSearchTerm !== lastScrolledTerm.current;
    ...
    if (isFreshSearch) {
      ...
      lastScrolledTerm.current = debouncedSearchTerm;
    }
  }, [debouncedSearchTerm, filteredGlobalStats, groupedResults]);
  ...
  return (
    <div className="min-h-screen p-6 text-gray-100 bg-gray-900 md:p-12">
          ...
          <div className="space-y-6">
            ...
            <div className="sticky top-0 z-40 bg-gray-900 -mx-6 px-6 pt-6 pb-6 border-b border-gray-800 shadow-md transition-all">
              <div className="flex flex-col gap-4">
                ...
                {/* ROW 1: Save/Load and Search Navigation */}
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                  ...
                  {/* Right: Search Field with Navigation (Find behavior) */}
                  <div className="w-full md:w-[420px] flex items-center bg-gray-800/50 border border-gray-700 rounded-xl focus-within:ring-1 focus-within:ring-spine-accent/50 focus-within:border-spine-accent/50 transition-all overflow-hidden">
                    ...
                    <input
                      ...
                      className="flex-1 py-2.5 px-3 text-sm text-gray-200 bg-transparent border-none focus:outline-none focus:ring-0 placeholder:text-gray-600"
                    />
                    
                    {/* Navigation Buttons and Counter */}
                    {searchTerm && (
                      <div className="flex items-center gap-1 pr-1 border-l border-gray-700 ml-1 bg-gray-800/20">
                        {searchMatches.length > 0 && (
                          <div className="flex items-center">
                            <span className="text-[10px] font-mono text-gray-500 px-2 whitespace-nowrap">
      ...
      <PercentageOverrideModal
        ...
        onClose={() => overrideModalOpen && setOverrideModalOpen(false)}
